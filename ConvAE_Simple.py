...
def CAE_Simple(input_shape=(28, 28, 1), filters=[32, 64, 10]):
    model = Sequential()
    ...
    model.add(Dense(units=filters[1]*int(input_shape[0]/4)*int(input_shape[0]/4), activation='relu'))

    model.add(Reshape((int(input_shape[0]/4), int(input_shape[0]/4), filters[1])))
...
if __name__ == "__main__":
    ...
    # define the model
    model = CAE_Simple(input_shape=x.shape[1:], filters=[32, 64, 10])
