...
class DCEC(object):
    ...
    def pretrain(self, x, batch_size=256, epochs=200, optimizer='adam', save_dir='results/temp'):
        ...
        csv_logger = CSVLogger(args.save_dir + '/pretrain_log_clusteringLoss.csv')

        # begin training
        t0 = time()
        self.cae.fit(x, x, batch_size=batch_size, epochs=epochs, callbacks=[csv_logger])
        print('Pretraining time: ', time() - t0)
        self.cae.save(save_dir + '/pretrain_cae_model_mnist_clusteringLoss.h5')
        print('Pretrained weights are saved to %s/pretrain_cae_model_mnist_clusteringLoss.h5' % save_dir)
        self.pretrained = True
    ...
    def fit(self, x, y=None, batch_size=256, maxiter=2e4, tol=1e-3,
        ...
        for ite in range(int(maxiter)):
            ...
            if ite % save_interval == 0:
                ...
                print('saving model to:', save_dir + '/dcec_model__mnist_1.0_clusteringLoss' + str(ite) + '.h5')
                self.model.save_weights(save_dir + '/dcec_model__mnist_1.0_clusteringLoss' + str(ite) + '.h5')

            ite += 1

        # save the trained model
        logfile.close()
        print('saving model to:', save_dir + '/dcec_model__mnist_final_1.0_clusteringLoss.h5')
        self.model.save_weights(save_dir + '/dcec_model__mnist_final_1.0_clusteringLoss.h5')
        t3 = time()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cae_weights', default="results/temp/pretrain_cae_model_mnist_clusteringLoss.h5", help='This argument must be given')
    ...
    # begin clustering.
    optimizer = 'adam'
    dcec.compile(loss=['kld', 'mse'], loss_weights=[0.5, 1], optimizer=optimizer)
